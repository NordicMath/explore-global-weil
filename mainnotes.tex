...
$$m(x_1) = \epsilon\frac{x_2\log(x_2)}{x_1(\log(x_1) + 2\pi in)}m(x_2)$$

Holds for:
$$x_2 = \exp(\frac{4\pi^2}{N}(\log(x_1) + 2\pi in)^{-1})$$

This is computationally confirmed.

However, there is somehow a second branch! We get the transform relation:
$$\log(x_2) + 2\pi im = \frac{4\pi^2}{N}(\log(x_1) + 2\pi in)^{-1}$$

I think this corresponds to the formula:
$$m(x_1) = \epsilon\frac{x_2(\log(x_2) + 2\pi im)}{x_1(\log(x_1) + 2\pi in)}m(x_2)$$


\subsection{A note on the transform}

We try to understand the transform:
$$f(x) = \exp(\frac{4\pi^2}{N}\log(x)^{-1})$$
It is an involution!

We investigate:
$$f(x^n) = \exp(\frac{4\pi^2}{N}\log(x^n)^{-1})$$
$$ = \exp(\frac{4\pi^2}{N}(n\log(x))^{-1})$$
$$ = \exp(\frac{4\pi^2}{N}n^{-1}\log(x)^{-1})$$
$$ = \exp(\frac{4\pi^2}{N}\log(x)^{-1})^{n^{-1}}$$
$$ = f(x)^{1/n}$$
And hence: 
$$f(x)^n = f(x^{1/n})$$
This gives importantly for later:
$$f(x)^{-1} = f(x^{1/-1}) = f(x^{-1})$$

\subsection{A second FE on some points}
...
$$\exp(\frac{4\pi^2}{N}\log(x)^{-1}) = 1/x$$
$$\frac{4\pi^2}{N}(\log(x) + 2\pi mi)^{-1} = \log(1/x) + 2\pi ni$$
$$\frac{4\pi^2}{N}(\log(x) + 2\pi mi)^{-1} = -(\log(x) + 2\pi ni)$$
$$-\frac{4\pi^2}{N} = (\log(x) + 2\pi mi)(\log(x) + 2\pi ni)$$
$$-\frac{4\pi^2}{N} = (\log(x)^2 + (2\pi ni + 2\pi mi)\log(x) + 2\pi ni2\pi mi)$$
$$-\frac{4\pi^2}{N} = \log(x)^2 + 2\pi i(n + m)\log(x) - 4\pi^2nm$$
$$\log(x)^2 + 2\pi i(n + m)\log(x) - 4\pi^2nm + \frac{4\pi^2}{N} = 0$$
$$\log(x)^2 + 2\pi i(n + m)\log(x) - 4\pi^2(nm - \frac{1}{N}) = 0$$
Let $\alpha = 2\pi i$
$$\log(x)^2 + \alpha(n + m)\log(x) + \alpha^2(nm - \frac{1}{N}) = 0$$
We use abc-formula:
$$\frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$$
We see $a = 1$ and $b = \alpha(n + m)$ and $c = \alpha^2(nm + \frac{1}{N})$
We input this:
$$\frac{-\alpha(n + m) \pm \sqrt{(\alpha(n + m))^2 - 4\alpha^2(nm - \frac{1}{N})}}{2}$$
$$\frac{-\alpha(n + m) \pm \sqrt{\alpha^2(n + m)^2 - 4\alpha^2(nm - \frac{1}{N})}}{2}$$
$$\frac{-\alpha(n + m) \pm \alpha\sqrt{(n + m)^2 - 4(nm - \frac{1}{N})}}{2}$$
$$\alpha\frac{-(n + m) \pm \sqrt{n^2 + 2nm + m^2 - 4(nm - \frac{1}{N})}}{2}$$
$$\alpha\frac{-(n + m) \pm \sqrt{n^2 - 2nm + m^2 + \frac{4}{N}}}{2}$$
$$\alpha\frac{-(n + m) \pm \sqrt{(n - m)^2 + \frac{4}{N}}}{2}$$
...
\subsection{Rambling42}

We have: 
$$\Lambda(s, \chi) = N^{s/2} \Gamma_\R(s + a) L(s, \chi)$$
$$\Lambda(s, \chi) = \epsilon \Lambda(1 - s, \overline{\chi})$$

We have: 
$$\Gamma_\R(s + a) = \pi^{-(s + a)/2} \Gamma\left(\frac{s + a}{2}\right)$$

So we get:
$$\Lambda(s, \chi) = N^{s/2} \pi^{-(s + a)/2} \Gamma\left(\frac{s + a}{2}\right) L(s, \chi)$$
$$ = N^{s/2} \pi^{-s/2} \pi^{-a/2} \Gamma\left(\frac{s + a}{2}\right) L(s, \chi)$$
$$ = \pi^{-a/2} \left(\frac{\pi}{N}\right)^{-s/2} \Gamma\left(\frac{s + a}{2}\right) L(s, \chi)$$
$$ = \pi^{-a/2} \left(\sqrt{\frac{\pi}{N}}\right)^{-s} \Gamma\left(\frac{s + a}{2}\right) L(s, \chi)$$

We see from the duplication formula that:
$$\Gamma\left(\frac{s + 1}{2}\right)\Gamma\left(\frac{s}{2}\right) = 2^{1 - s} \sqrt{\pi} \Gamma(s)$$
$$ = \Gamma\left(\frac{s + a}{2}\right)\Gamma\left(\frac{s + (1 - a)}{2}\right)$$

This gives us:
$$\Gamma\left(\frac{s + a}{2}\right) = 2^{1 - s} \sqrt{\pi} \Gamma(s) \Gamma\left(\frac{s + (1 - a)}{2}\right)^{-1}$$

So we get:
$$\Lambda(s, \chi) = \pi^{-a/2} \left(\sqrt{\frac{\pi}{N}}\right)^{-s} 2^{1 - s} \sqrt{\pi} \Gamma(s) \Gamma\left(\frac{s + (1 - a)}{2}\right)^{-1} L(s, \chi)$$
$$\Lambda(s, \chi) = 2\pi^{-a/2} \pi^{1/2} \left(\sqrt{\frac{\pi}{N}}\right)^{-s} 2^{-s} \Gamma(s) \Gamma\left(\frac{s + (1 - a)}{2}\right)^{-1} L(s, \chi)$$
$$\Lambda(s, \chi) = 2\pi^{(1-a)/2} \Gamma\left(\frac{s + (1 - a)}{2}\right)^{-1} \left(2\sqrt{\frac{\pi}{N}}\right)^{-s} \Gamma(s) L(s, \chi)$$
Define $b = 1 - a$ and we get:
$$\Lambda(s, \chi) = 2\pi^{b/2} \Gamma\left(\frac{s + b}{2}\right)^{-1} \left(2\sqrt{\frac{\pi}{N}}\right)^{-s} \Gamma(s) L(s, \chi)$$
$$\Lambda(s, \chi)\Gamma\left(\frac{s + b}{2}\right) \frac{\pi^{-b/2}}{2} =  \left(2\sqrt{\frac{\pi}{N}}\right)^{-s} \Gamma(s) L(s, \chi)$$
Extract: 
$$\Gamma\left(\frac{s + b}{2}\right)$$
And do $s \mapsto 1 - s$. We get:
$$\Gamma\left(\frac{(1 - s) + b}{2}\right)$$
$$ = \Gamma\left(\frac{1}{2} + \frac{-s}{2} + \frac{b}{2}\right)$$
$$ = \Gamma\left(1 - \frac{s + 1}{2} + \frac{b}{2}\right)$$
$$ = \Gamma\left(1 - \frac{s + 1 - b}{2}\right)$$
$$ = \Gamma\left(1 - \frac{s + a}{2}\right)$$
$$ = \Gamma\left(1 - \frac{s + a}{2}\right)$$
...
$$\exp(\frac{4\pi^2}{N}\log(x)^{-1}) = 1/x$$
$$\frac{4\pi^2}{N}\log(x)^{-1} = -\log(x)$$
$$\frac{4\pi^2}{N} = -\log(x)^2$$
$$\pm \sqrt{-\frac{4\pi^2}{N}} = \log(x)$$
$$\exp(\pm \sqrt{-\frac{4\pi^2}{N}}) = x$$
$$x = \exp(\pm i\frac{2\pi}{\sqrt{N}})$$

Here we get: 
$$m(x) = \epsilon\frac{1/x\log(1/x)}{x\log(x)}m(1/x)$$
$$m(x) = \epsilon\frac{-1/x\log(x)}{x\log(x)}m(1/x)$$
$$m(x) = \epsilon\frac{-1}{x^2}m(1/x)$$
$$x^2m(x) = -\epsilon m(1/x)$$
$$m(1/x) = -\epsilon x^2m(x)$$


AMAZING!!!!!!

Conjecture: This holds for all $x$

\subsection{Rambling49}

We do it again for branches:
...
Say $g = g_1 + g_2$ and say:
$$g_1(x) = \frac{x^2}{f(x)^2} g_1(f(x))$$
$$g_2(x) = \frac{x^2}{f(x)^2} g_2(f(x))$$
We add these together:
$$g_1(x) + g_2(x) = \frac{x^2}{f(x)^2} g_1(f(x)) + \frac{x^2}{f(x)^2} g_2(f(x))$$
$$g_1(x) + g_2(x) = \frac{x^2}{f(x)^2} (g_1(f(x)) + g_2(f(x)))$$
$$g(x) = \frac{x^2}{f(x)^2} g(f(x))$$
Also, say $g = Cg_1$. And say:
$$g_1(x) = \frac{x^2}{f(x)^2} g_1(f(x))$$
We multiply by $C$:
$$Cg_1(x) = \frac{x^2}{f(x)^2} Cg_1(f(x))$$
$$g(x) = \frac{x^2}{f(x)^2} g(f(x))$$
We have that this $FE$ is linear!!! We test $g(x) = x^n$. We get:
$$g(x) = \frac{x^2}{f(x)^2} g(f(x))$$
$$x^n = \frac{x^2}{f(x)^2} f(x)^n$$
$$x^{n - 2} = f(x)^{n - 2}$$
If $n = 2$ then okay. Else we get:
$$|x| = |f(x)|$$
This is not true in general. Is this enough? Maybe... We test something else. Assume:
$$g(x) = \sum_{n = 0} b_n x^n$$
We set this in:
$$\sum_{n = 0} b_n x^n = \frac{x^2}{f(x)^2} \sum_{n = 0} b_n f(x)^n$$
$$\sum_{n = 0} b_n x^{n - 2} = \sum_{n = 0} b_n f(x)^{n - 2}$$
$$\sum_{n = 0} b_n (x^{n - 2} - f(x)^{n - 2}) = 0$$
Assume $b_n$ is a solution. Since $c_n = 0$ but $c_2 = b_2$ is also a solution, we have $b_n - c_n$ also a solution. Hence we can set $b_2 = 0$. We get:
$$b_0 (x^{-2} - f(x)^{-2}) + b_1 (x^{-1} - f(x)^{-1}) + \sum_{n = 3} b_n (x^{n - 2} - f(x)^{n - 2}) = 0$$

We try another path: 
Back to FE:
$$g(x) = \frac{x^2}{f(x)^2} g(f(x))$$
We set:
$$x = \exp\left(y\frac{2\pi}{\sqrt{N}}\right)$$
$$f(x) = \exp\left(y^{-1}\frac{2\pi}{\sqrt{N}}\right)$$
We get:
$$g(\exp\left(y\frac{2\pi}{\sqrt{N}}\right)) = \frac{( \exp\left(y\frac{2\pi}{\sqrt{N}}\right))^2}{(\exp\left(y^{-1}\frac{2\pi}{\sqrt{N}}\right))^2} g(\exp\left(y^{-1}\frac{2\pi}{\sqrt{N}}\right))$$


NOTE!!!! We know:
$$m(1/x) = -\epsilon x^2 m(x)$$
For: 
$$x = e^{\pm 2\pi i \frac{1}{\sqrt{N}}}$$
So:
$$g(x) = -\epsilon x^2$$
For this point.

We also know this for: 
$$x = \exp(\pm 2\pi i \sqrt{n^2 - \frac{1}{\sqrt{N}}})$$
This is a converging sequence, it has a limit point, so it follows from the identity theorem! This works for meromorphic!!!!




\subsection{Rambling52 COME BACK HERE}

We multiply instead:
$$m(1/x)m(x) = \epsilon\frac{x\log(f(x))}{f(x)\log(x)}m(1/f(x))\epsilon\frac{f(x)\log(f(x))}{x\log(x)} m(f(x))$$
$$m(1/x)m(x) = \epsilon^2\frac{\log(f(x))}{\log(x)}\frac{\log(f(x))}{\log(x)} m(1/f(x)) m(f(x))$$
$$m(1/x)m(x) = \frac{\log(f(x))^2}{\log(x)^2}m(1/f(x)) m(f(x))$$
Say that $m(1/x) = g(x)m(x)$. Then:
$$g(x)m(x)m(x) = \frac{\log(f(x))^2}{\log(x)^2}g(f(x))m(f(x)) m(f(x))$$
$$g(x)m(x)^2 = \frac{\log(f(x))^2}{\log(x)^2}g(f(x))m(f(x))^2$$
